...
class AVLTree:
    ...
    def searchTreeName(self, data): #Public version of the search function, return true or false when asked about a data number
        if self.root != None: #If the root exists then continue onto the private version
            return self._searchTreeName(data, self.root)
        else: #Otherwise return false since no root means no tree and no tree means no nodes anywhere
            return False

    def _searchTreeName(self, data, currentNode):
        if data == currentNode.country: #If the searched term is equal to the currentNode's data then return true
            return True
        elif data < currentNode.country and currentNode.left != None: #Else if the data is less than the currentNode's value and the currentNode has a left child, then recursively check left side
            return self._searchTreeName(data, currentNode.left)
        elif data > currentNode.country and currentNode.right != None: #Do the same thing if it is greater except on the right side of the tree
            return self._searchTreeName(data, currentNode.right)
        return False #If everything goes through and true is still not returned then return false since we know that it won't exist
...
class minHeap:
    ...
    def search(self, name):
        ...
        while (self.size > 0 or ans == True):
            removeNode = self.Pop()
            if removeNode.nameOfCountry == name:
                ans = True
        return ans
...
def moreInfo():
    treeTime = getBigTree()
    minHeapTime = getBigMinHeap()
    _countryT = ""
    _countryH = ""
    _name = ""
    if request.method == 'POST':
        number = request.form['countryNumber']
        _name = "Country " + str(number)
        if (treeTime.searchTreeName(_name)):
            countryT = "Found node"
        else:
            countryT = "Invalid node"
        if (minHeapTime.search(_name)):
            countryH = "Found node"
        else:
            countryH = "Invalid node"
