...
class AVLTree:
    ...
    def searchCountry(self, countryName):
        if self.root != None:
            return self._searchCountry(countryName, self.root)
        else:
            return False
    
    def _searchCountry(self, countryName, currentNode):
        if countryName == currentNode.country:
            return True
        elif countryName < currentNode.country and currentNode.left != None:
            return self._searchCountry(countryName, currentNode.left)
        elif countryName > currentNode.country and currentNode.right != None:
            return self._searchCountry(countryName, currentNode.right)
        return False
...
class minHeap:
    ...
    def search(self, name):
        ans = False
        for i in range (0, (self.size+1) ):            
            if self.Heap[i].nameOfCountry == name:
                ans = True
        return ans
...
def specialStructuresSearch():
    ...
    start = time.time()
    ans = specialTree.searchTree(country.data)
    end = time.time()
